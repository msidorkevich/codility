 ...
 * The number 15 has binary representation 1111 and has no binary gaps.Write a function:
...
public class BinaryGap {
    ...
    public static int gapSize(int n) {
        int firstOne = firstOne(n);
        int lastOne = lastOne(n);
    ...
    private static int firstOne(int n) {
        for (int i = 0; i <= numberOfNonZeroBits(n); i++) {
            if (getBit(n, i) == 1) {
                return i;
            }
        }
    ...
    private static int lastOne(int n) {
        for (int i = numberOfNonZeroBits(n); i >= 0; i--) {
            if (getBit(n, i) == 1) {
                return i;
            }
        }
    ...
    private static int numberOfNonZeroBits(int n) {
        return (int) Math.round(Math.ceil(log(n) / log(2)));
    }
}
